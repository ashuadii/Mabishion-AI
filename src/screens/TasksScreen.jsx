      ...
      <div className="space-y-8">
        <section className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <p className="tagline text-[10px] font-bold" style={{ color: C.goldDeep }}>Kanban</p>
            <h1 className="mt-3 font-heading text-5xl" style={{ color: C.navyDeep }}>Task command board</h1>
          ...
          <div className="flex flex-wrap gap-3">
            {QUEUE.map(item => (
              <div key={item.label} className="rounded-2xl border bg-white/70 px-5 py-4" style={{ borderColor: C.glassBorder }}>
                <p className="text-xs font-bold uppercase" style={{ color: C.textMuted }}>{item.label}</p>
                <p className="mt-1 font-heading text-3xl" style={{ color: C.primary }}>{item.value}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid gap-5 xl:grid-cols-4">
          {COLUMNS.map((column) => (
            <div key={column.title} className="rounded-[28px] border bg-white/60 p-5" style={{ borderColor: C.glassBorder }}>
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-heading text-3xl" style={{ color: C.primary }}>{column.title}</h2>
                  <p className="mt-2 text-sm leading-6" style={{ color: C.textMuted }}>{column.summary}</p>
                </div>
              ...
              <div className="space-y-4">
                {column.tasks.map((task) => (
                  <article key={task.title} className="group rounded-2xl border bg-white/76 p-4 transition-all hover:-translate-y-1 hover:bg-white" style={{ borderColor: C.glassBorder }}>
                    <div className="mb-4 flex items-center justify-between gap-2">
                      <Badge tone={task.due === 'Today' ? 'danger' : 'gold'}>{task.due}</Badge>
                      <Icon name="drag_indicator" size={18} style={{ color: C.muted }} />
                    </div>
                    <h3 className="text-base font-bold leading-6" style={{ color: C.navyDeep }}>{task.title}</h3>
                    <div className="mt-4 space-y-2 text-sm" style={{ color: C.textMuted }}>
                      <p>{task.owner}</p>
                      <p>{task.impact}</p>
                    </div>
                  </article>
        ...
        <section className="rounded-[28px] border p-6" style={{ borderColor: C.glassBorder, background: C.navyDeep }}>
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-white/68">Revenue, client communication, deployment, and credential actions stay in review until an operator clears the gate.</p>
            </div>
