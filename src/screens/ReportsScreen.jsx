...
import { KPI_CARDS } from '../data/mockData.jsx';

function Sparkline({ points, tone = "gold" }) {
  const color = tone === "success" ? C.success : tone === "cyan" ? C.cyanBright : tone === "danger" ? C.danger : C.softGold;
  const w = 160, h = 56;
  ...
  return (
    <AppShell activeNavId="reports" onNavigate={onNavigate}
      ...
      <section className="grid grid-cols-12 gap-5">
        {/* KPI Cards */}
        <div className="col-span-12 p-5" style={glassStyle({ strong: true, glow: "gold" })}>
          <div className="mb-5 flex items-center justify-between"><h3 className="font-black">KPI Dashboard</h3><Badge tone="gold">Reality signals</Badge></div>
            ...
            {KPI_CARDS.map((kpi) => (
              <div key={kpi.label} className="rounded-[22px] p-5" style={{ backgroundColor: "rgba(255,255,255,.04)", border: `1px solid ${kpi.tone === "danger" ? `${C.danger}35` : C.glassBorder}` }}>
                <div className="mb-4 flex items-center justify-between">
                  <Icon name={kpi.icon} size={22} style={{ color: kpi.tone === "success" ? C.success : kpi.tone === "cyan" ? C.cyanBright : kpi.tone === "danger" ? C.danger : C.softGold }} />
                  <Badge tone={kpi.tone}>{kpi.change}</Badge>
                </div>
                <p className="text-4xl font-black" style={{ color: kpi.tone === "danger" ? C.danger : kpi.tone === "success" ? C.success : kpi.tone === "cyan" ? C.cyanBright : C.softGold }}>{kpi.value}</p>
        ...
        {/* Weekly Report */}
        <div className="col-span-8 p-5" style={glassStyle({ glow: "cyan" })}>
          ...
          <div className="mb-5 rounded-[24px] p-5" style={{ backgroundColor: `${C.gold}10`, border: `1px solid ${C.gold}33` }}>
            <p className="text-xs font-black uppercase tracking-[0.16em]" style={{ color: C.softGold }}>Verdict</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              { title: "Root cause", body: "Marketing is creating attention, but handoff speed into Leads and Projects is still inconsistent.", tone: "violet" },
              { title: "Next action", body: "Fix Lead Engine approval gate first, then run one Showroom proof campaign for 48 hours.", tone: "gold" },
            ].map((section) => (
              <div key={section.title} className="rounded-[22px] p-5"
                style={{ backgroundColor: section.tone === "danger" ? `${C.danger}10` : section.tone === "success" ? `${C.success}0F` : section.tone === "violet" ? `${C.violetBright}10` : `${C.gold}10`, border: `1px solid ${C.glassBorder}` }}>
                <div className="mb-3 flex items-center gap-2">
                  <Icon name={section.tone === "danger" ? "warning" : section.tone === "success" ? "check" : section.tone === "violet" ? "target" : "sparkles"} size={18} style={{ color: section.tone === "danger" ? C.danger : section.tone === "success" ? C.success : section.tone === "violet" ? C.violetBright : C.softGold }} />
                  <p className="font-black">{section.title}</p>
                </div>
                <p className="text-sm leading-6" style={{ color: C.muted }}>{section.body}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-4 space-y-5">
          <div className="p-5" style={glassStyle({ glow: "gold" })}>
            ...
            <div className="space-y-3">
              {[{ title: "Package Proposal OS", score: 87, reason: "Small paid starter product. Low build effort, clear buyer pain." }, { title: "Showroom Case Study", score: 82, reason: "Proof asset can lift trust before pricing." }].map((item, i) => (
                <div key={item.title} className="rounded-[20px] p-4" style={{ backgroundColor: i === 0 ? `${C.gold}10` : "rgba(255,255,255,.04)", border: `1px solid ${i === 0 ? `${C.gold}44` : C.glassBorder}` }}>
                  ...
                  <p className="text-xs leading-5" style={{ color: C.mutedLow }}>{item.reason}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="p-5" style={glassStyle({ glow: "violet" })}>
            ...
            <div className="space-y-3">
              {[{ label: "Revenue", points: [38, 44, 41, 62, 58, 72, 78], tone: "gold" }, { label: "Leads", points: [52, 48, 65, 61, 70, 66, 73], tone: "cyan" }, { label: "Projects", points: [25, 32, 40, 43, 51, 60, 71], tone: "success" }].map((line) => (
                <div key={line.label} className="rounded-[18px] p-3" style={{ border: `1px solid ${C.glassBorder}`, backgroundColor: "rgba(255,255,255,.04)" }}>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <Badge tone={line.tone === "cyan" ? "cyan" : line.tone}>{line.points[line.points.length - 1]}%</Badge>
                  </div>
