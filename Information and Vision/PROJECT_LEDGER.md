...
**2026-07-04 — Owner Decision: Repository renamed Nexious → Mabishion.** `~/Desktop/Nexious-AI` → `~/Desktop/Mabishion-AI`; `Nexious Mickii/` → `Mabishion Mickii/`; `nexious-ai-starter/` → `mabishion-ai-starter/`. Backward-compat symlink `Nexious-AI → Mabishion-AI` left on Desktop (remove after confirming nothing references the old name). Updated: `.gitignore`, `CLAUDE.md`, `WORKSPACE_RULES.md` (paths). Git worktrees repaired; Claude memory copied to new project key. Code untouched — internal `nexious_cost_alert` event name and brand-leak validators intentionally preserved. Old duplicate snapshot moved to `~/Desktop/_DELETE_ME_Mabishion-AI-old-snapshot` (owner to delete). Build ✅, tests 54/54 ✅ at new path.
