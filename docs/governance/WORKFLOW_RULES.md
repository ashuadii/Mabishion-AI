...
| **5. Sync** | Punch `PROJECT_LEDGER.md` (format below). If an architecture fact changed, update `PROJECT_RULES.md` in the same session. **Then commit + push to origin/dev (standing owner authorization, 2026-07-15: every validated change auto-pushes — no per-push approval needed).** | Ledger entry exists. Docs match reality. Change is on GitHub. |
