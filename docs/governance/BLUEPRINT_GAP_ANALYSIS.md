...
| P4 | **Campaigns module (simulation mode)** — campaign CRUD, budget caps, ad-copy variants via workers, performance tracking; Meta/Google live APIs deferred as optional Phase 3. | High (owner is a marketer) | Largest (4–6 sessions) | **Medium** — big new surface; simulation-only keeps external risk at zero | ✅ Phase 1 shipped 2026-07-15 (schema v23: campaigns/campaign_ads/campaign_metrics; simulation engine with Meta/Google benchmarks + budget cap; CampaignSimulator UI in Sales & Marketing Hub). Next phases: LLM ad-variant generation via workers, metrics charts, live APIs (CRITICAL gate). |
